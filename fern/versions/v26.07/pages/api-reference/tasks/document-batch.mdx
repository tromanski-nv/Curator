...
<Note>
Returning `None` remains the supported way to filter a batch out. The resumability layer converts it to a `NoneTask`, which represents a filtered task and consumes that branch, so a `None` return is what you will see surfaced as `NoneTask` downstream. See [Resumable Processing](/reference/infra/resumable-processing).
</Note>
