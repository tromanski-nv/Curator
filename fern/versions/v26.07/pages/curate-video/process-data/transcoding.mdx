...
<Tip>
On systems with supported NVIDIA GPU hardware and an `ffmpeg` build with NVENC, `h264_nvenc` can significantly increase throughput. On GPUs without NVENC hardware, use `libvpx-vp9`, or opt in to `libopenh264` if H.264 output is required.
</Tip>
...
transcode = ClipTranscodingStage(
    encoder="h264_nvenc",        # or "libvpx-vp9", "libopenh264"
...
| --- | --- |
| `encoder` | Selects the encoding backend. Supported values are `h264_nvenc`, `libvpx-vp9`, and `libopenh264`. |
