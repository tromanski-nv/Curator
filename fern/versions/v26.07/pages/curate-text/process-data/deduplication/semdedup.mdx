...
workflow = TextSemanticDeduplicationWorkflow(
    ...
    n_clusters=100,
    kmeans_fit_data_fraction=0.1,  # Fit centroids on about 10% of input files
...
| `input_filetype` | str | `"parquet"` | Reader format. Supported values are `"parquet"` and `"jsonl"`. |
...
| `kmeans_fit_data_fraction` | float \| None | `None` | Fraction of files used to fit K-means in `TextSemanticDeduplicationWorkflow`; enables the memory-efficient two-pass path |
...
1. **Fit pass:** Each actor samples `round(fit_data_fraction * number_of_actor_files)` files, with a minimum of one file. It reads only the embedding column from those files and cooperatively fits the centroids.
2. **Predict and write pass:** Each actor reads every original file group, one group at a time, assigns every row to a centroid, computes centroid distances, writes the partitioned result, and releases the group before reading the next one.
