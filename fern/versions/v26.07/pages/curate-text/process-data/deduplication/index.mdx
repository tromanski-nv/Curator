...
The exact, fuzzy, semantic, and duplicate-removal workflows use `input_filetype` to select both the reader and the default file extensions. When `input_file_extensions` is omitted, the workflows discover these files:

| `input_filetype` | Default `input_file_extensions` | Files discovered |
| --- | --- | --- |
| `"parquet"` | `[".parquet"]` | Parquet files only |
| `"jsonl"` | `[".jsonl", ".json"]` | JSON Lines files that use either suffix |

Only `"parquet"` and `"jsonl"` are supported by these deduplication workflows. Other file types are rejected rather than falling back to the generic file partitioner defaults.

`input_file_extensions` follows these rules:

- `None` uses the defaults for `input_filetype`.
- A non-empty list replaces the defaults. Extensions can include or omit the leading dot and matching is case-insensitive.
- An empty list behaves like `None`; it does not disable file discovery.
- The extension selects files, but `input_filetype` still selects the reader. For example, `input_file_extensions=[".pq"]` with `input_filetype="parquet"` reads `.pq` files as Parquet.

For a single directory string, file discovery includes all nested subdirectories. For workflows that accept a list of input paths, each listed directory is scanned only at its top level; list individual files or directories when you do not want recursive discovery. Extension filtering also applies when an input path names a file directly.

<Tabs>
<Tab title="JSONL defaults">
...
# Discovers input_data/**/*.jsonl and input_data/**/*.json.
```

</Tab>
<Tab title="Custom Parquet suffix">
...
# Discovers input_data/**/*.pq and reads each file as Parquet.
```

</Tab>
</Tabs>

Use the same `input_filetype`, `input_file_extensions`, input paths, and partitioning settings for identification and `TextDuplicatesRemovalWorkflow` when IDs are generated from input partitions. Changing file discovery or partition boundaries can change generated IDs and prevent removal from resolving them.

When you supply prebuilt `initial_tasks`, the workflow skips file discovery. In that mode, input-path, extension, and partitioning options do not change the supplied tasks.
...
The removal workflow uses the same file-discovery rules as identification. If identification used a custom suffix, pass the same override when removing duplicates:
