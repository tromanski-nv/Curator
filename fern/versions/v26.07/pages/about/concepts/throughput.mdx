...
Use `RayClient` for single-node setups or `SlurmRayClient` for multi-node SLURM clusters:
...
<Note>
`SlurmRayClient` subclasses `RayClient`. On a single-node SLURM job, or when the script is not running under SLURM at all, it behaves identically to `RayClient` — so it is safe to use unconditionally in scripts that run in both settings.
</Note>
