...
`runtime_env=None` means no override and preserves the stage's current environment. Pass `runtime_env={}` to explicitly replace it with an empty environment dictionary.

The same `with_()` call can configure resources, worker counts, and backend stage specs. See [Stage Worker Sizing](/reference/infra/stage-worker-sizing) for the full signature and shallow-merge semantics.
