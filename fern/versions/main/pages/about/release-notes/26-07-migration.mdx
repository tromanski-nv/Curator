...
Standard pip cannot install `all` because it cannot apply Curator's dependency
overrides. Use the `uv pip install` command above, install a narrower supported
extra, or use the NeMo Curator container.
