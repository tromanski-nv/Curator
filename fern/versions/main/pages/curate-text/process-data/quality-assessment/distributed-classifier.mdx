...
First download the override file and add the CUDA indexes as shown in the
[installation guide](/get-started/installation). Standard pip is not supported
for this extra. When using classifiers that download from Hugging Face (such as
Aegis and InstructionDataGuard), set your `HF_TOKEN` environment variable to
avoid rate limiting:
